...
class CountTaskBoxStyle: UIView {
    let label = LabelStyle(text: "Количество задач", font: .systemFont(ofSize: 25))
    ...
    func getInfo() {
        ApiCaller.instance.getCountTask(uidUser: Auth.auth().currentUser!.uid) { (count) in
